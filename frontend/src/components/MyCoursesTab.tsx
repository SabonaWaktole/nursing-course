...
export default function MyCoursesTab() {
    ...
    return (
        <div className="flex flex-col gap-8">
            <motion.div variants={sectionContainer} initial="hidden" animate="show" className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
            ...
            {enrollments.length === 0 ? (
                <motion.div variants={sectionItem} initial="hidden" animate="show" className="text-center py-20 rounded-2xl border-2 border-dashed border-slate-200/50 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl">
                    ...
                    <p className="text-sm text-slate-400 mb-6">Start your learning journey by enrolling in a course from the navigation menu.</p>
                </motion.div>
            ) : (
                <>
                    {activeEnrollment && (
                        <motion.div variants={sectionContainer} initial="hidden" whileInView="show" viewport={viewportOnce} className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                            <motion.div variants={sectionItem} className="lg:col-span-2">
                                <motion.div initial="hidden" whileInView="show" viewport={viewportOnce} variants={imageInViewVariants} className="h-full">
                            ...
                            <motion.div variants={sectionItem} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-slate-200/50 dark:border-slate-800/50 flex flex-col gap-5 shadow-[0_4px_20px_rgba(15,23,42,0.04)] hover:shadow-[0_8px_30px_rgba(15,23,42,0.08)] hover:-translate-y-0.5 transition-all duration-300">
                                ...
                                <div className="flex items-end justify-between h-24 px-1">
                                    {weeklyActivity.map((h, i) => (
                                        <motion.div key={i} initial={{ height: 0 }} whileInView={{ height: `${h}%` }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.08, ease: [0.25, 0.8, 0.25, 1] }} className={`w-5 rounded-lg ${h > 0 ? 'bg-gradient-to-t from-primary to-cyan-400 shadow-[0_0_8px_rgba(13,185,242,0.3)]' : 'bg-primary/15'}`} />
                                    ))}
                                </div>
                    ...
                    <motion.div variants={sectionContainer} initial="hidden" whileInView="show" viewport={viewportOnce} className="flex flex-col gap-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                            {enrollments.map((enrollment, idx) => (
                                <motion.div key={enrollment.id} variants={sectionItem} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
                                    <Link href={`/courses/${enrollment.course.id}`} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl overflow-hidden border border-slate-200/50 dark:border-slate-800/50 group shadow-[0_4px_20px_rgba(15,23,42,0.04)] hover:shadow-[0_12px_35px_rgba(15,23,42,0.12)] transition-all duration-300 block h-full">
                                        <div className="relative h-40 overflow-hidden">
                                            <motion.div
                                                initial="hidden" whileInView="show" viewport={viewportOnce} variants={imageInViewVariants}
                                                className="absolute inset-0 bg-center bg-cover group-hover:scale-110 transition-transform duration-700"
                                                style={{ backgroundImage: `url('${enrollment.course.thumbnail ? getFileUrl(enrollment.course.thumbnail) : COURSE_IMAGES[idx % COURSE_IMAGES.length]}')` }}
                                            />
                                        ...
                                        <div className="p-5 flex flex-col gap-4">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
                                                    <motion.div initial={{ width: 0 }} whileInView={{ width: `${enrollment.progress}%` }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.3, ease: [0.25, 0.8, 0.25, 1] }} className={`h-full rounded-full ${enrollment.progress === 100 ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : 'bg-gradient-to-r from-primary to-cyan-400'} progress-bar-glow`} />
                                                </div>
                        ...
                        <motion.div variants={sectionContainer} initial="hidden" whileInView="show" viewport={viewportOnce} className="flex flex-col gap-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {enrollments.filter(e => e.completed).map((enrollment) => (
                                    <motion.div key={enrollment.id} variants={sectionItem} whileHover={{ y: -2 }} className="flex items-center justify-between p-5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200/50 dark:border-slate-800/50 shadow-[0_4px_20px_rgba(15,23,42,0.04)] hover:shadow-[0_8px_30px_rgba(15,23,42,0.08)] transition-all duration-300">
                                        ...
                                        <motion.span whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                                            <Link href={`/my-learning?tab=certificates`} className="flex items-center gap-1.5 text-primary font-bold text-xs bg-primary/10 px-3.5 py-2 rounded-xl hover:bg-primary/20 transition-all">
